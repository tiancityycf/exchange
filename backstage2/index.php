...
$CFG->action = str_replace("[^a-zA-Z_\-]", "",$_REQUEST['action']);
$CFG->bypass = ($_REQUEST['bypass'] || $CFG->print);
$CFG->is_tab = (!$CFG->url) ? 1 : $_REQUEST['is_tab'];
//$CFG->id = ereg_replace("[^0-9]", "",$_REQUEST['id']);
$CFG->id = str_replace("[^0-9]", "",$_REQUEST['id']);
$CFG->target_elem = str_replace("[^a-zA-Z_\-]", "",$_REQUEST['target_elem']);
...
<?	
	...
	elseif ($CFG->url == 'my-account') {
		...
		$form_name = str_replace("[^a-zA-Z_\-]", "",$_REQUEST['form_name']);
...
elseif (User::isLoggedIn() && (User::$info['verified_authy'] == 'Y' && !($_SESSION['token_verified'] > 0))) {
	...
	$l_form->info['user'] = str_replace("[^0-9a-zA-Z!@#$%&*?\.\-\_]", "",$l_form->info['user']);
	$l_form->info['pass'] = str_replace("[^0-9a-zA-Z!@#$%&*?\.\-\_]", "",$l_form->info['pass']);
