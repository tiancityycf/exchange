		...
		if ($controls) {
			foreach ($controls as $c_id => $control) {
				...
				if (!$is_static) {
					...
					//Control::$class = $ref->newInstanceArgs($args);
...
/*
							Control::$class->verify();
							Control::$class->save();
							Control::$class->show_errors();
							Control::$class->show_messages();
							Control::$class->get(($page_info['url'] == 'my-account' || $url == 'my-account') ? User::$info['id'] : $_REQUEST['id']);
*/
						}
						else {
							$this->class->get($CFG->include_id);
							//Control::$class->get($CFG->include_id);
						}
						$this->class->info['p_id'] = $_REQUEST['p_id'];
						$this->class->info['p_id'] = $_REQUEST['p_id'];
						//Control::$class->info['f_id'] = $_REQUEST['f_id'];
						//Control::$class->info['f_id'] = $_REQUEST['f_id'];

						if ($page_info['url'] == 'my-account' || $url == 'my-account') {
							$CFG->o_method_suppress = true;
							$this->class->passiveField('id','ID');
							//Control::$class>passiveField('id','ID');
							$CFG->o_method_suppress = true;
							$this->class->textInput('user',$CFG->user_username,true,false,false,false,false,false,false,false,1,$CFG->user_unique_error);
							//Control::$class->textInput('user',$CFG->user_username,true,false,false,false,false,false,false,false,1,$CFG->user_unique_error);
							$CFG->o_method_suppress = true;
							$this->class->passwordInput('pass',$CFG->user_password,true);
							//Control::$class->passwordInput('pass',$CFG->user_password,true);
							$CFG->o_method_suppress = true;
							$this->class->passwordInput('pass1',$CFG->user_password,true,false,false,false,false,false,'pass');
							//Control::$class->passwordInput('pass1',$CFG->user_password,true,false,false,false,false,false,'pass');
							$CFG->o_method_suppress = true;
							$this->class->textInput('first_name',$CFG->user_first_name,true);
							//Control::$class->textInput('first_name',$CFG->user_first_name,true);
							$CFG->o_method_suppress = true;
							$this->class->textInput('last_name',$CFG->user_last_name,true);
							//Control::$class->textInput('last_name',$CFG->user_last_name,true);
							$CFG->o_method_suppress = true;
							$this->class->textInput('phone',$CFG->user_phone);
							//Control::$class->textInput('phone',$CFG->user_phone);
							$CFG->o_method_suppress = true;
							$this->class->textInput('email',$CFG->user_email);
							//Control::$class->textInput('email',$CFG->user_email);
							if (User::$info['is_admin'] == 'Y') {
								$CFG->o_method_suppress = true;
								$this->class->selectInput('f_id',$CFG->user_group,false,$_REQUEST['f_id'],false,'admin_groups',array('name'));
								//Control::$class->selectInput('f_id',$CFG->user_group,false,$_REQUEST['f_id'],false,'admin_groups',array('name'));
								$CFG->o_method_suppress = true;
								$this->class->checkBox('is_admin',$CFG->user_is_admin);
								//Control::$class->checkBox('is_admin',$CFG->user_is_admin);
							}
						}
					}
				...
				if (is_array($control['methods'])) {
					foreach ($control['methods'] as $method) {
						...
						$inputs_array = Control::getSubMethods($method['id'],$class);
						...
						if ($is_static) {
							...
							//新增 if判断
//							if(is_object(Control::$class)){
							//		$method_instance->invokeArgs(Control::$class,$args);
//							}
						}
					}
				}
				...
				if ($class == 'Form' && !$CFG->pm_decouple_cancel) {
					...
					//Control::$class->cancelButton($CFG->cancel_button);
				}
				...
				if (!$is_static) {
						if ($class == 'Grid'){
								...
								//Control::$class->display($_REQUEST['page'.Control::$class->i]);
						}else{
								$this->class->display();
/*
								if(!is_bool(Control::$class)){
...
*/
	...
	public static function getCurrentTabId($url,$is_tab=false) {
		...
		$row = Control::getPageId($url,$is_tab,1);
