...
class SString {
	...
	public static function fauxArray($array,$not_root=false) {
		...
		foreach ($array as $k=>$v) {
			if (@strlen($v) > 0 || is_array($v)) {
				if (is_array($v)) {
					$ret[] = $k.'=>('.SString::fauxArray($v,true).')';
				}
				else {
					$ret[] = $k.'=>'.$v;
				}
	...
	public static function stripQuotes($v) {
		if (is_array($v))
	    	return array_map(array('SString','stripQuotes'), $v);
	  	else
	...
	public static function magicQuotesOff() {
		if (get_magic_quotes_gpc()) {
  			$_GET = SString::stripQuotes($_GET);
  			$_POST = SString::stripQuotes($_POST);
  			$_REQUEST = SString::stripQuotes($_REQUEST);
  			$_COOKIES = SString::stripQuotes($_COOKIES);
		}
	}
	...
	public static function truncate($text, $length = 100, $ending = '...', $exact = false, $considerHtml = true) {
		...
		if ($considerHtml) {
			...
			foreach ($lines as $line_matchings) {
				...
				if ($total_length+$content_length> $length) {
					...
					SString::$missing = substr($line_matchings[2],$left+$entities_length);
		...
		if (!$exact) {
			...
			if (isset($spacepos)) {
				...
				SString::$missing = substr($truncate,$spacepos).SString::$missing;
	...
	function doFormulaReplacements($formula,$row=false,$directly_link_fields=false,$use_provided_row=false) {
		...
		$matches = SString::getSubstring($formula,'[',']');
	...
	public static function replaceConditionals($formula,$row,$f_id_field=false) {
		...
		if (is_array($conditionals[2])) {
			...
			foreach ($conditionals[2] as $conditional) {
				$matches = SString::getSubstring($return_values[2][$i],'[',']');
				$matches = (count($matches) > 0) ? $matches : array();
				$matches1 = SString::getSubstring($conditional,'[',']');
				...
				$matches = SString::getSubstring($conditional,'[',']');
				if ($matches) {
					foreach ($matches as $match) {
						if (strstr($match,',')) {
							$match_parts = explode(',',$match);
		...
		if (is_array($variables)) {
			foreach ($variables as $k => $v) {
				...
				if (strstr($v1,'(') && strstr($v1,')')) {
					$formula = SString::doFormulaReplacements($v1,$row,1);
