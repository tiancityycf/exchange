...
class MultiList {
	...
	function display() {
		...
		$HTML = '
		<div class="area full_box multi_list '.$this->class.'" id="mlist_'.self::$i.'">
			<h2>'.$title.'</h2>
			...
			foreach ($this->levels as $level => $value) {
				...
				foreach ($value as $j) {
					...
					'.( (!$CFG->bypass) ? '$(document).ready(function(){' : '' ).'
						$(".mlist_'.self::$i.'_level_'.$level.'").sortable({
									revert: true,
									cursor: "move",
									opacity: 0.7,
									items: ".li_'.$level.'",
									connectWith: ".mlist_'.self::$i.'_level_'.$level.'"			
						}); 
					'.( (!$CFG->bypass) ? '});' : '' ).'
					';
				}
	...
	private function displayItems($tables,$rows=false,$f_id=0,$level=1,$p_id=0) {
		...
		if ($tables) {
			...
			foreach ($tables as $name => $properties) {
				$HTML .= "<ul id=\"mlist_{self::$i}_level_{$level}_{$parent_id}\" class=\"mlist_{self::$i}_level_{$level}\">";
