...
class User {
	...
	public static function logIn($user=false,$pass=false,$table=false,$session_name=false) {
		...
		$user = strip_tags(mysqli_real_escape_string($DB_CONN,$user));
		$pass = strip_tags(mysqli_real_escape_string($DB_CONN,$pass));
	...
	public static function verify($user,$pass,$table=false) {
		...
		if (SString::sanitize($user,true,true)) {
			return false;
		}
			
		if (SString::sanitize($pass,true,true)) {
			return false;
		}
