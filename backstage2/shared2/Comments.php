...
class Comments {
	...
	function get($p_id=0,$count=false) {
		...
		if ($sql_filter) {
			$matches = SString::getSubstring($sql_filter,'[',']');
		...
		if ($sql_filter) {
			$sql_filter = SString::doFormulaReplacements($sql_filter);
			$sql .= " AND (".$sql_filter.') ';
		}
	...
	private function show($comments) {
		...
		if ($comments) {
			...
			foreach ($comments as $comment) {
				...
				$action = SString::doFormulaReplacements($action,unserialize($comment['f_table_row']),1,1);
