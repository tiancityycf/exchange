...
function db_connect($dbhost, $dbname, $dbuser, $dbpass,$dbport) {
	global $DB_DIE_ON_FAIL, $DB_DEBUG, $CFG,$DB_CONN;
	
	if (! $dbh = mysqli_connect ( $dbhost, $dbuser, $dbpass,$dbname,$dbport )) {
		if ($DB_DEBUG) {
	...
	//echo "---conn----";
	/*
	   $result = mysqli_query($dbh,"select * from account_types");
...
/*
	if (!$CFG->dont_select_db) {
		if (! mysqli_select_db ( $DB_CONN,$dbname )) {
			if ($DB_DEBUG) {
...
function db_query($query, $debug = false, $die_on_debug = true, $silent = false, $unbuffered = false) {
	...
	if ($unbuffered)
		...
		$qid = mysqli_query ($DB_CONN, $query );
	
	if (! $qid && ! $silent) {
		if ($DB_DEBUG) {
			echo "<h2>Can't execute query</h2>";
			echo "<pre>" . htmlspecialchars ( $query ) . "</pre>";
			echo "<p><b>MySQL Error</b>: ", mysqli_error ($DB_CONN);
			echo "<p><b>Debug</b>: ";
...
function db_fetch_array($qid, $type = MYSQLI_NUM) {
	return mysqli_fetch_array ( $qid, $type );
}
