...
class Settings {
	...
	public static function set($name, $value) {
		...
		$affected_rows = db_update ( 'settings', $name, array ('value' => htmlentities($value) ), 'name' );
		
		if (! $affected_rows) {
			db_insert ( 'settings', array ('name' => $name, 'value' => htmlentities($value) ), '', true );
		}
	...
	public static function getAll() {
		...
		if ($result) {
			foreach ($result as $key => $row) {
				$result[$key]['value'] = SString::checkSerialized(html_entity_decode($row['value']));
			}
		}
		return $result;
	}
	...
	public static function getStructured() {
		...
		if ($result) {
			foreach ($result as $row) {
				$name = $row['name'];
				$row['value'] = SString::checkSerialized(html_entity_decode($row['value']));
				if (!is_array($row['value'])) {
					$ret[$name] = (stristr($row['value'],'"')) ? htmlentities($row['value']) : $row['value'];
				}
				else {
					$ret[$name] = SString::fauxArray($row['value']);
				}
			}
		}
		return $ret;
