...
function db_connect($dbhost, $dbname, $dbuser, $dbpass,$dbport) {
	...
	if (class_exists('PDO')) {
		try {
			$dbh = new PDO('mysql:host='.$dbhost.';port='.$dbport.';dbname='.$dbname.';charset=utf8', $dbuser, $dbpass,array(PDO::ATTR_EMULATE_PREPARES => false,PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
		} 
        ...
        $output .= "MySQL Error: need PDO open";
        trigger_error($output,E_USER_ERROR);
		return $dbh;
	}
}
